...
void classRF2(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	     ...
	     int *outclts, int *labelts, double *proxts, double *errts,
             int *inbag,int *areas, int *nareas) {
    /******************************************************************
     *  *miinbag es MORCILLA
	...
	*at, *a, *b, *mind, *nind, *jts, *oobpair;
    ...
    selAreas =       (int *) S_alloc(*sampsize, sizeof(int));
    if (oobprox) {
		...
		do {
			...
			if (stratify) {  /* stratified sampling */
				...
				do {
					...
					zeroDouble(tclasspop, nclass);
					zeroDouble(win, nsample);
                                        zeroInt(selAreas,*nareas); 
					if (replace) {
                                            if(nareas>0){
                                                //warning("HOLA %d %d %d %d %d %d %d\n",*nareas,areas[3],areas[4],areas[5],areas[6],areas[7],*(areas+8));
                                                k2=0;
                                                for (n = 0; n < *nareas; ++n){  /* Este bucle hace bootstrapping de áreas*/
                                                        k= (int) (unif_rand() *  *nareas) + 1;
                                                        // printf("area: %d\n",k);
                                                        for (n2=0;n2<nsample;++n2){ /* Incluye en selAreas todos los pixels del area*/
                                                                          //printf("       %d %d %d\n",n2,*(areas+n2),k);
                                                               if(*(areas+n2)==k && k2<*sampsize){selAreas[k2]=n2;k2++;
                                                                          //printf("               %d  %d\n",n2,areas[n2]);
                                                               }
                                                        }
                                                }
