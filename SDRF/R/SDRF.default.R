...
#' Function for random forest classification with spatial dependent training data
#'
#' @param x points
#' @param y morepoints
#' @param xtest cosicas
#' @param ytest mas cosicas
#' @param ntree 500
#' @param mtry if (!is.null(y) && !is.factor(y)) max(floor(ncol(x)/3), 1) else floor(sqrt(ncol(x)))
#' @param replace TRUE
#' @param classwt NULL
#' @param cutoff corta
#' @param strata strato
#' @param sampsize = if (replace) nrow(x) else ceiling(.632*nrow(x))
#' @param nodesize = if (!is.null(y) && !is.factor(y)) 5 else 1
#' @param maxnodes = NULL
#' @param importance FALSE
#' @param localImp FALSE
#' @param nPerm 1
#' @param proximity ds
#' @param oob.prox proximity
#' @param norm.votes TRUE
#' @param do.trace FALSE
#' @param keep.forest !is.null(y) && is.null(xtest)
#' @param corr.bias FALSE
#' @param keep.inbag FALSE
#' @param areas areas
#' @param ... otros
#' @export
#' @import randomForest
#' @useDynLib SDRF classRF2
"SDRF.default" <-
    ...
    if (classRF) {
        #miinbag=sample(1:n,n,replace=T)
        #cat(n,"   ",sort(miinbag),"\n")
        if (!is.null(areas)){ nareas=max(areas)} else {nareas=0}
        cwt <- classwt
        threshold <- cutoff
